...
- **Locais reais** (nome, categoria, coordenadas) do **OpenStreetMap**, via Overpass API — gratuita, sem chave, sem surpresas de limite de uso. Consegui 1.479 locais reais no Porto (bares, restaurantes, cafés, pubs, discotecas).
...
1. **`OverpassInAdapter`** (Inbound Adapter) — a cada 3 segundos, pega um lote de ~150 locais da lista carregada de `data/porto_venues.json`, calcula a popularidade sintética de cada um pro instante atual, e entrega o lote pro Service.
